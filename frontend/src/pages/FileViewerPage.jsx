...
const FileViewer = () => {
  ...
  useEffect(() => {
    const fetchRecentlyViewed = async () => {
      ...
      try {
        const res = await fetch(`${API_BASE_URL}/api/files/recently-viewed/${user.firebaseUser.uid}?limit=4`);
  ...
  const addToRecentlyViewed = async (fileData) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/files/recently-viewed`, {
        method: 'POST',
        headers: {
      ...
      if (res.ok) {
        ...
        const refreshRes = await fetch(`${API_BASE_URL}/api/files/recently-viewed/${user.firebaseUser.uid}?limit=4`);
  ...
  useEffect(() => {
    ...
    const fetchFile = async () => {
      ...
      setPreviewData(null);
      try {
        const detailsRes = await fetch(`${API_BASE_URL}/api/files/${selectedFileId}/details`);
        ...
        if (fileDetails.fileType === 'pdf') {
          const viewResponse = await fetch(`${API_BASE_URL}/api/files/${selectedFileId}/view`, { headers });
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true);
    try {
      const response = await fetch(`${API_BASE_URL}/api/files/${file._id}`, { method: 'DELETE' });
